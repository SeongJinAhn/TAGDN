...
class HetGDC(nn.Module):
    ...
    def forward(self, X, edge_index, neg_edge):
        ...
        # Type-Information Restoration
        Z = self.type_infomation_restoration(tilde_Z, mean_node_type, std_node_type)
        # We define the link existence probability as the simlarity between type-independent information (tilde_Z)
        return tilde_Z, self.TIMReg(tilde_H, edge_index, neg_edge).cuda()
